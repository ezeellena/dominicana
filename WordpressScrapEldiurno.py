...
def obtenerTituloRss(item):
    ...
    try:
        Titulo = limpiar(item["title"],mugre)
        return Titulo
...
def obtenerImagenRss(item):
    try:
        ...
        print("No es necesario obtener la pagina", e)
    try:
                    ...
                    try:
                        ...
                        for link in links:
                            ...
                            if not Titulo or not Descripcion or not Imagen or not TextoCompleto or not Categoria or not FechaPublicacion:
                                ...
                                try:
                                    user = 'admin'
                                    pythonapp = 'GFBJ yX1j JqYI mzz6 Hg4a wuh6'
                                    url = 'http://el-diurno.com/wp-json/wp/v2'
                                    data_string = user + ':' + pythonapp
                                    token = base64.b64encode(data_string.encode())
                                    headers = {'Authorization': 'Basic ' + token.decode('utf-8')}
                                    imgsrc = Imagen
